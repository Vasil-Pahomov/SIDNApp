 ...
 *
 * http://www.java2s.com/Open-Source/Android_Free_Code/Development/data/org_sesy_coco_datacollector_audioConvolution_java.htm
 *
 * @author siniwitt
 */
public class Convolution {
    ...
    public Convolution(double[] filterKernel, int signalLength){
        ...
        this.signal = new double[2*newL];
        this.h = new double[2*newL];
        System.arraycopy(filterKernel, 0, this.h, 0, N);
        tmp = new double[2*newL];
    }
     ...
     * @param resultSignal will contain the resulting samples, ensure it is big enough by using the getFrameSize() method.
     */
    public void computeConvResult(double[] origSignal, double[] resultSignal){
        for (int i=K;i<signal.length;i++) {
            signal[i] = 0;
        }
        ...
        double[] h_temp = new double[this.h.length];
        System.arraycopy(this.h, 0, h_temp, 0, N);

        //fft
        fft.realForwardFull(signal);
        fft.realForwardFull(h_temp);


        //complex mult.
        for(int i = 0; i < newL; i++){
            tmp[2*i]        = signal[2*i]*h_temp[2*i]   - signal[2*i+1]*h_temp[2*i+1];
            tmp[2*i+1]      = signal[2*i]*h_temp[2*i+1] + signal[2*i+1]*h_temp[2*i];
        }
